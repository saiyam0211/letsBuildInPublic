...
export interface ServerToClientEvents {
  ...
  'job-completed': (result: any) => void;
  'job-failed': (error: string) => void;
  'queue-stats': (stats: any) => void;
...
export interface ClientToServerEvents {
  ...
  'get-job-progress': (jobId: string, callback: (progress: JobProgress | null) => void) => void;
  'get-user-jobs': (callback: (jobs: JobProgress[]) => void) => void;
  'cancel-job': (jobId: string, callback: (success: boolean) => void) => void;
  'get-queue-stats': (callback: (stats: any) => void) => void;
}
...
export class WebSocketService {
  ...
  private setupEventHandlers(): void {
    ...
    this.io.on('connection', (socket: Socket<ClientToServerEvents, ServerToClientEvents, InterServerEvents, SocketData>) => {
      ...
      logger.info(`User ${userEmail} (${userId}) connected with socket ${socket.id}`);
      ...
      socket.on('leave-project-room', (projectId: string) => {
        ...
        socket.data.rooms = socket.data.rooms.filter(room => room !== `project:${projectId}`);
  ...
  emitJobProgress(progress: JobProgress): void {
    ...
    logger.debug(`Emitted job progress for ${progress.jobId}: ${progress.progress}% - ${progress.currentStep}`);
  }

  /**
   * Emit job completion
   */
  emitJobCompleted(userId: string, projectId: string, result: any): void {
    if (!this.io) return;
    ...
    logger.info(`Emitted job completion for user ${userId}, project ${projectId}`);
  }
   ...
   */
  notifyUser(userId: string, message: string, type: 'info' | 'success' | 'warning' | 'error' = 'info'): void {
    if (!this.io) return;
   ...
   */
  broadcast(event: keyof ServerToClientEvents, data: any): void {
    if (!this.io) return;
   ...
   */
  getServer(): Server<ClientToServerEvents, ServerToClientEvents, InterServerEvents, SocketData> | null {
    return this.io;
  }
  ...
  async shutdown(): Promise<void> {
    if (this.io) {
      ...
      this.broadcast('notification', 'Server is shutting down. Please reconnect in a moment.');
