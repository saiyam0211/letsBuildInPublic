...
const riskSchema = new Schema<IRisk>({
  type: {
    ...
    enum: {
      ...
      message: 'Risk type must be one of: market, technical, financial, competitive'
    }
  },
  description: {
    type: String,
...
const ideaValidationSchema = new Schema<IIdeaValidation>({
  ...
  differentiationOpportunities: [{
    type: String,
    trim: true,
    minlength: [10, 'Differentiation opportunity must be at least 10 characters long'],
    maxlength: [300, 'Differentiation opportunity cannot exceed 300 characters']
  }],
  risks: {
    type: [riskSchema],
    default: [],
...
ideaValidationSchema.pre('validate', function(this: IIdeaValidation) {
  if (this.differentiationOpportunities && this.differentiationOpportunities.length > 10) {
...
export const IdeaValidation = mongoose.model<IIdeaValidation>('IdeaValidation', ideaValidationSchema); 