...
externalIntegrationSchema.methods.checkRateLimit = function (
  this: IExternalIntegration
): boolean {
  const now = new Date();
  ...
  } else if (now.getTime() - resetTime.getTime() > 3600000) {
    // 1 hour
    this.rateLimits.currentUsage.hour = 0;
    this.rateLimits.currentUsage.minute = 0;
  } else if (now.getTime() - resetTime.getTime() > 60000) {
    // 1 minute
    this.rateLimits.currentUsage.minute = 0;
  }
...
externalIntegrationSchema.methods.incrementUsage = function (
  this: IExternalIntegration
) {
...
externalIntegrationSchema.methods.getSuccessRate = function (
  this: IExternalIntegration
): number {
  if (this.statistics.totalSyncs === 0) return 100;
  return Math.round(
    (this.statistics.successfulSyncs / this.statistics.totalSyncs) * 100
  );
};

export const ExternalIntegration = mongoose.model<IExternalIntegration>(
  'ExternalIntegration',
