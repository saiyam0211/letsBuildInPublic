...
export interface IDiagram extends Document {
  ...
  type: 'user-flow' | 'system-architecture' | 'database-schema' | 'api-endpoints' | 'sequence' | 'class' | 'custom';
...
const diagramSchema = new Schema<IDiagram>({
  ...
  type: {
    ...
    enum: {
      values: ['user-flow', 'system-architecture', 'database-schema', 'api-endpoints', 'sequence', 'class', 'custom'],
      message: 'Type must be one of: user-flow, system-architecture, database-schema, api-endpoints, sequence, class, custom'
    }
  },
  content: {
    type: String,
